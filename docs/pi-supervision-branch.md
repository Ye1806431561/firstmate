...
Ordinary main-only rows remain on main even when eligible task-local rows share their queue, except that a decision-owned or completed-scout signal or stale trigger keeps its entire coalesced trigger batch on main.
...
- Wake dispatch: `.pi/extensions/fm-primary-pi-watch.ts` stays the dispatcher; `.pi/extensions/lib/fm-branch-dispatch.ts` owns the offer handshake and row eligibility, while [`watcher-continuity.md`](watcher-continuity.md#per-actor-acknowledgement) owns the per-actor consume contract.
  ...
  Under the away-posture record the check-kind, decision-owned, and completed-scout exclusions lift and every actionable row is offered ("Postures" below), while the no-acceptor fallback and the alarms still reach main.
  ...
  While attended, `scopeForUnreadWake` also excludes a scout whose latest complete status event is `done`, so its report review, captain-call completion gate, and guarded cleanup remain one main-owned lifecycle; a still-working scout remains branch-eligible.
  The status read ignores trailing explanatory prose, honors a later real event, withholds an unterminated terminal event, and refuses a symlink or mid-read change rather than guessing.
  The dispatcher resolves trigger keys and every currently unread attended-main-owned row to task identity before cross-referencing them: any signal or stale trigger containing a decision-owned or completed-scout task goes wholly to main, including a batch that also contains routine rows, and an unread row for one task keeps every later signal or stale trigger for that same task on main until the row is read, regardless of whether the rows use its status-file key or window alias.
...
- Autonomy: supervision is default-on for every task once a Pi primary session owns the fleet lock (docs/configuration.md "Pi supervision branch"); no captain grant file is required.
  A fleet-wide heartbeat is separately eligible only when every row other than an attended-main-owned check, decision, or completed-scout row is a heartbeat row or a resolvable task-local row (see "Heartbeat routing" below); every other fleet-wide or unresolvable wake, and every watcher-failure alarm, stays on main.
...
A heartbeat is never vetoed or ridden into main by a co-present check row, decision-owned signal or stale row, or completed-scout row.
...
- Every actionable row is branch-eligible: check rows, decision-owned or completed-scout signal and stale rows, and heartbeat rows are claimed by the branch on whatever wake finds them unread, and the trigger class no longer forces a batch to main.
