...
Every scout signal or stale trigger also stays on main, which owns report delivery, the captain-call completion gate, and guarded cleanup as one lifecycle.
...
- Wake dispatch: `.pi/extensions/fm-primary-pi-watch.ts` stays the dispatcher; `.pi/extensions/lib/fm-branch-dispatch.ts` owns the offer handshake and row eligibility, while [`watcher-continuity.md`](watcher-continuity.md#per-actor-acknowledgement) owns the per-actor consume contract.
  ...
  It also excludes every signal or stale row whose current task metadata says `kind=scout`, leaving the main session to follow the scout-completion contract in `AGENTS.md` section 7 rather than accepting a branch outcome as the end of fleet handling.
  The dispatcher resolves trigger keys and every currently unread main-owned row to task identity before cross-referencing them: any signal or stale trigger containing a main-owned task goes wholly to main, including a batch that also contains routine rows, and an unread main-owned row for one task keeps every later signal or stale trigger for that same task on main until the row is read, regardless of whether the rows use its status-file key or window alias.
...
- Autonomy: supervision is default-on for every task once a Pi primary session owns the fleet lock (docs/configuration.md "Pi supervision branch"); no captain grant file is required.
  A fleet-wide heartbeat is separately eligible only when every row other than a check or main-owned signal/stale row (a decision event or scout lifecycle event) is a heartbeat row or a resolvable task-local row (see "Heartbeat routing" below); every other fleet-wide or unresolvable wake, and every watcher-failure alarm, stays on main.
  ...
  Main-owned scout rows follow that same independent-row rule, while a signal or stale trigger for the scout itself stays on main.
...
A heartbeat is never vetoed or ridden into main by a co-present check row or main-owned signal/stale row (a decision event or scout lifecycle event).
...
Its direct dispatch classifier case covers decision-owned and scout signal/stale rows' exclusion from `eligibleSeqs`, their presence in `mainOwnedKeys`, the decision subset in `needsDecisionKeys`, task alias resolution, reserved-key configuration, status-log race and symlink refusal, non-vetoing behavior for unrelated eligible rows, and main-only queues reading as ordinary absence.
...
The branch-offer, scout-completion-stays-on-main, heartbeat-offer, heartbeat-not-ridden-by-main-only-rows, main-only-check-class, captain-held-stale-stays-on-main, and mixed-signal-routing tests remain in `tests/fm-pi-watch-extension.test.sh` (the scout and last two routing classes exercise `offerWakeToBranch`'s trigger-key cross-reference end to end), the recovery test remains in `tests/fm-session-start.test.sh`, and the per-actor consume regression remains in `tests/fm-wake-queue.test.sh`.
