...
Scout signal and stale rows stay in this conversation so their report delivery, captain-call completion check, and guarded cleanup remain one task lifecycle under `AGENTS.md` section 7.
Decision-owned and scout signal/stale routing, including whole-batch precedence and the independent heartbeat exception, is owned by [docs/pi-supervision-branch.md](../pi-supervision-branch.md#components-and-their-owners).
